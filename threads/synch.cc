...
void Lock::Release() {
	...
	if(lockOwner != currentThread) {
		printf("\n\nThe Thread trying to release the lock is not the owner");	
		interrupt->SetLevel(oldLevel);
		return;
	}
	removedThread = (Thread *)lockQueue->Remove();
	if (removedThread != NULL) {
		scheduler->ReadyToRun(removedThread);
		lockOwner = removedThread;
	}
	else {
		lockStatus = FREE;
		lockOwner = NULL;
...
void Condition::Wait(Lock* conditionLock) { 
	...
	if(waitingLock != conditionLock) {
		printf("\n\nCondition Lock & Waiting Lock Different");
		interrupt->SetLevel(oldLevel);
		return;		
	}
...
void Condition::Signal(Lock* conditionLock) { 
	...
	removedThread = (Thread *)lockWaitQueue->Remove();
	if (removedThread == NULL) {
		interrupt->SetLevel(oldLevel);
		return;
	}
...
int Condition::Broadcast(Lock* conditionLock) { 
	int a=0;
	while (!lockWaitQueue->IsEmpty() )          //while there are waiting threads
	{
		a++;
